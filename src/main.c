...
int main(){
	...
	pipe(pipegreet);
	pipe(piperoot);
	struct passwd *glassdmuser=getpwnam("glassdm");
	if(!glassdmuser){
	...
	if(greeterroot==0){
		close(piperoot[0]);
		int ret=pam_start("glassdmgreeter","glassdm",&greeterconv,&pamgreeter);
		printf("%d\n",ret==PAM_SUCCESS);
		...
		int pidc=fork();
		if(pidc==0){
			initgroups("glassdm",glassdmuser->pw_uid);
			setgid(glassdmuser->pw_uid);
			setuid(glassdmuser->pw_uid);
			execve("/usr/bin/kwin_wayland",argv,envp);
			return 0;
		}
		char waylandpath[64];
		sprintf(waylandpath,"/run/user/%d/wayland-0",glassdmuser->pw_uid);
		while(access(waylandpath,F_OK)){
			sleep(1);
		}
		char *argv2[]={"/var/lib/glassdm/greeter",NULL};
		int pidc2=fork();
		if(pidc2==0){
			dup2(pipegreet[1],78);
			close(pipegreet[0]);
			chdir("/var/lib/glassdm/");
			initgroups("glassdm",glassdmuser->pw_uid);
			setgid(glassdmuser->pw_uid);
			setuid(glassdmuser->pw_uid);
			execve("/var/lib/glassdm/greeter",argv2,envp);
			return 0;
		}
		close(pipegreet[1]);
		char temp=(char)1;
		auth:
			...
			ret=pam_start("glassdmlogin",datain.name,&conv,&pamlogin);
	...
	close(piperoot[0]);
	waitpid(greeterroot,&status,0);
	int ret=pam_start("glassdmlogin",datain.name,&greeterconv,&pamlogin);
